...
def run_image_viewer(file_path=""):
    ...
    color_map = "Greys_r"  # for grayscale images

    # Function to open an image
    def open_image():
        nonlocal img, points, file_path
    ...
    def scale_subplot():
        nonlocal original_xlim, original_ylim, color_map, white_point, image_origin, image_scale
        ...
        # Calculate the xy axis based on origin and mm/pixel
        # X-axis
        if img is not None:
    ...
    def clear_point_plot():
        nonlocal color_map, white_point
    ...
    def on_click(event):
        ...
        if zoom_state or pan_state:
            print("Measurement inhibited when panning or zooming")
            return
    ...
    image_scale_entry.insert(0, "0.1")  # Default value
    ...
    update_button = tk.Button(root, text="Reset Zoom", command=restore_zoom)
    update_button.pack()

    # Create a Figure with a specific size (width, height) in inches
    fig = Figure(figsize=(9, 7))  # Adjust the width and height as needed
...
# When this script is run directly, it opens the viewer
